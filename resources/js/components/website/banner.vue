              ...
              >
                ...
                <div class="col-md-12">
                  <div class="form-group">
                    ...
                    <vs-select v-model="item.type" class="w-100" @input="onTypeChange(item)">
                      ...
                      <vs-select-item value="video" text="Video (tải lên từ máy)" />
                    </vs-select>
                  </div>
                </div>
                ...
                <div class="col-md-9">
                  <div class="form-group" v-if="item.type === 'video'">
                    <label>Video banner <span class="text-danger">*</span></label>
                    <p class="text-muted small mb-2">
                      Tải video từ máy tính (MP4, WebM, MOV — tối đa 80MB).
                    </p>
                    <video-upload v-model="item.video_url"></video-upload>
                  </div>
  ...
  methods: {
    ...
    normalizeBanner(item) {
      return {
        ...defaultBanner(),
        ...item,
        type: item.type === "youtube" || item.type === "video" ? "video" : "image",
        video_url: this.isUploadedVideoPath(item.video_url) ? item.video_url : "",
        status: String(
          item.status !== undefined && item.status !== null ? item.status : 1
        ),
      };
    },
    isUploadedVideoPath(url) {
      const value = (url || "").trim();
      if (!value) return false;
      if (/youtube\.com|youtu\.be/i.test(value)) return false;
      if (/^[a-zA-Z0-9_-]{11}$/.test(value)) return false;
      return true;
    },
    onTypeChange(item) {
      if (item.type === "image") {
    ...
    validateBanners() {
      for (let i = 0; i < this.objData.length; i++) {
        ...
        if (item.type === "video") {
          if (!this.isUploadedVideoPath(item.video_url)) {
            this.$error(`Banner #${i + 1}: Vui lòng tải video từ máy lên.`);
            return false;
          }
        } else if (!item.image) {
